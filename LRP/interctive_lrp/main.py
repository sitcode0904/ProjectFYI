...
def interactive_lrp(config: argparse.Namespace):
    ...
    if not os.path.isfile("C:\\Users\\Ankoor\\Documents\\brain_tumor_dataset\\no\\no 923.jpg"):
        print(f"Error: The file at locATION does not exist.")
        return

    # Read the input image
    image = cv2.imread("C:\\Users\\Ankoor\\Documents\\brain_tumor_dataset\\no\\no 923.jpg")
    if image is None:
        print(f"Error: Unable to load image at location")
        return
...
if __name__ == "__main__":
    ...
    # Prompt user for input path if not provided
    if not config.input_path:
        config.input_path = input("Please enter the path to the input image: ")
